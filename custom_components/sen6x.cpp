...
void SEN6XComponent::schedule_post_setup_commands_() {
  ...
  // FIX: Circular reference memory leak removed. Replaced with clean ESPHome set_interval.
  auto index_ptr = std::make_shared<size_t>(0);
  this->set_interval("sen6x_setup_steps", 20, [this, steps, index_ptr]() {
    if (*index_ptr >= steps->size()) {
      this->cancel_interval("sen6x_setup_steps");
      this->finish_setup_();
      return;
    }
    (*steps)[*index_ptr]();
